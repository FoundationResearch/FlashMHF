...
@triton.autotune(configs=_CONFIGS,
                 key=["seq_len", "expert_dim", "head_dim"],
                 prune_configs_by={"early_config_prune": _prune_configs})
@triton.jit
def flash_mlp_moe_forward_kernel(
    # Input tensors
    Q_ptr, K_ptr, U_ptr, V_ptr, R_ptr,
    # Output tensor
    O_ptr,
    # Dimensions
    batch_size, num_heads, seq_len, head_dim, num_expert, expert_dim,
    # Strides for Q (batch_size, num_heads, seq_len, head_dim)
    q_batch_stride, q_head_stride, q_seq_stride, q_head_dim_stride,
    # Strides for K (num_kuv_heads, num_expert, expert_dim, head_dim)
    k_head_stride, k_expert_stride, k_expert_dim_stride, k_head_dim_stride,
    # Strides for U (num_kuv_heads, num_expert, expert_dim, head_dim)
    u_head_stride, u_expert_stride, u_expert_dim_stride, u_head_dim_stride,
    # Strides for V (num_kuv_heads, num_expert, expert_dim, head_dim)
    v_head_stride, v_expert_stride, v_expert_dim_stride, v_head_dim_stride,
    # Strides for R (batch_size, num_heads, seq_len, num_expert)
    r_batch_stride, r_head_stride, r_seq_stride, r_expert_stride,
    ...
    tl.assume(EXPERT_DIM % 16 == 0)
    tl.assume(EXPERT_DIM % BLOCK_INTER == 0)               # expert dim must be a multiple of block inter
    ...
    # expert_ids = tl.arange(0, NUM_EXPERT)[None, :] # [E]
    ...
    r_ptr = R_ptr + batch_id * r_batch_stride + q_head_id * r_head_stride
    ...
    q_block = tl.load(q_ptr_block, mask=mask_rows) # (NUM_KUV_GROUPS * BLOCK_SEQ, HEAD_DIM)
    r_ptr_block = (
        r_ptr
        + g * r_head_stride
        + s[:, None] * r_seq_stride
    ...
    for expert_id in tl.range(0, NUM_EXPERT, 1):
        expert_score = tl.load(r_ptr_block + expert_id * r_expert_stride) # (NUM_KUV_GROUPS * BLOCK_SEQ, 1)
        for einter_start in tl.range(0, EXPERT_DIM, BLOCK_INTER):
            einter_start = tl.multiple_of(einter_start, BLOCK_INTER)
            expert_dim_idx = einter_start + row_inter
            k_ptr_block = k_ptr + expert_id * k_expert_stride + expert_dim_idx * k_expert_dim_stride
            u_ptr_block = u_ptr + expert_id * u_expert_stride + expert_dim_idx * u_expert_dim_stride
            v_ptr_block = v_ptr + expert_id * v_expert_stride + expert_dim_idx * v_expert_dim_stride
            ...
            v_block = tl.load(v_ptr_block)            # (BLOCK_INTER, HEAD_DIM), no bdry check, no need to transpose
            ...
            # Compute Q @ U^T
            qu_scores = tl.dot(q_block, u_block) # (NUM_KUV_GROUPS * BLOCK_SEQ, BLOCK_INTER)
            activated_scores = (activated_scores * qu_scores * expert_score).to(tl.bfloat16)
...
def _prune_bwd_configs(configs, named_args, **meta):
    seq_len = named_args["seq_len"]
    intermediate_size = named_args["num_expert"] * named_args["expert_dim"]
    head_dim = named_args["head_dim"]
    has_up = True  # up projection always present
    valid = []
    for conf in configs:
...
@triton.autotune(configs=_BWD_CONFIGS,
                 key=["seq_len", "num_expert", "expert_dim", "head_dim"],
                 prune_configs_by={"early_config_prune": _prune_bwd_configs})
@triton.jit
def flash_mlp_moe_backward_kernel_dqdr(
    # Input tensors
    Q_ptr, K_ptr, U_ptr, V_ptr, R_ptr, dS_ptr,
    # Output gradients
    dQ_ptr, dR_ptr,
    # Dimensions
    batch_size, num_heads, seq_len, head_dim, num_expert, expert_dim,
    ...
    # dk_head_stride, dk_expert_stride, dk_expert_dim_stride, dk_head_dim_stride,
    # du_head_stride, du_expert_stride, du_expert_dim_stride, du_head_dim_stride,
    # dv_head_stride, dv_expert_stride, dv_expert_dim_stride, dv_head_dim_stride,
    BLOCK_SEQ: tl.constexpr,
    BLOCK_INTER: tl.constexpr,
    HEAD_DIM: tl.constexpr,
    NUM_KUV_GROUPS: tl.constexpr,
    ...
    tl.assume(k_expert_dim_stride % 16 == 0)
    tl.assume(u_expert_dim_stride % 16 == 0)
    tl.assume(v_expert_dim_stride % 16 == 0)
    tl.assume(q_batch_stride % 16 == 0)
    tl.assume(ds_batch_stride % 16 == 0)
    tl.assume(EXPERT_DIM % BLOCK_INTER == 0)
    ...
    r_ptr = (
        R_ptr + batch_id * r_batch_stride + q_head_id * r_head_stride
        + g * r_head_stride
        + s * r_seq_stride
    )
    ...
    dr_ptr = dR_ptr + batch_id * r_batch_stride + q_head_id * r_head_stride
    ...
    for expert_id in tl.range(0, NUM_EXPERT, 1):
        expert_score = tl.load(r_ptr + expert_id * r_expert_stride, mask=mask_rows).to(tl.bfloat16) # (NUM_KUV_GROUPS * BLOCK_SEQ, 1)
        dr_rows = tl.zeros((NUM_KUV_GROUPS * BLOCK_SEQ, 1), dtype=tl.float32)
        for inter_start_ in tl.range(0, EXPERT_DIM, BLOCK_INTER, num_stages=None):
            tl.multiple_of(inter_start_, BLOCK_INTER)
            expert_dim_idx = inter_start_ + row_inter
            k_ptr_block_1 = k_ptr + expert_id * k_expert_stride + expert_dim_idx * k_expert_dim_stride
            u_ptr_block_1 = u_ptr + expert_id * u_expert_stride + expert_dim_idx * u_expert_dim_stride
            v_ptr_block_1 = v_ptr + expert_id * v_expert_stride + expert_dim_idx * v_expert_dim_stride
            ...
            vT_block_1 = tl.trans(tl.load(v_ptr_block_1, cache_modifier='.cg'))
            ...
            dr_rows += tl.sum(dA * siluM * N, axis=1)[:, None]
            dM = (dA * (expert_score * N)) * (siluM + sigM*(1.0-siluM))
            dN = (dA * siluM) * expert_score
        ...
        dr_rows_block = dr_ptr + g * r_head_stride + s * r_seq_stride + expert_id * r_expert_stride
        tl.store(dr_rows_block, dr_rows.to(tl.bfloat16), mask=mask_rows)
...
@triton.autotune(configs=_BWD_CONFIGS,
                 ...
                 prune_configs_by={"early_config_prune": _prune_bwd_configs})
@triton.jit
def flash_mlp_moe_backward_kernel_dkdudv(
    # Input tensors
    Q_ptr, K_ptr, U_ptr, V_ptr, R_ptr, dS_ptr,
    # Output gradients
    ...
    # Constants
    RCP_LN2: tl.constexpr = 1.4426950408889634  # = 1.0 / ln(2)
    TILES_PER_EXP: tl.constexpr = EXPERT_DIM // BLOCK_INTER
    
    # Program IDs
    pid0 = tl.program_id(0)
    expert_id = pid0 // TILES_PER_EXP
    block_id = pid0 % TILES_PER_EXP
    kuv_head_id = tl.program_id(1)
    ...
    g = (row_ids >> LOG2_BLOCK_SEQ)[:, None]  # row_ids // BLOCK_SEQ
    s_local = row_ids & (BLOCK_SEQ - 1)       # row_ids % BLOCK_SEQ
    expert_dim_idx = inter_start + row_inter
    # Pointers for current block
    q_ptr = (
    ...
    k_ptr = K_ptr + kuv_head_id * k_head_stride + col_ids * k_head_dim_stride
    u_ptr = U_ptr + kuv_head_id * u_head_stride + col_ids * u_head_dim_stride
    v_ptr = V_ptr + kuv_head_id * v_head_stride + col_ids * v_head_dim_stride
    r_ptr = (
        R_ptr + q_head_id * r_head_stride
        + g * r_head_stride
        + expert_id * r_expert_stride
    )
    ds_ptr = (
    ...
    tl.multiple_of(k_ptr, [16,16])
    tl.multiple_of(u_ptr, [16,16])
    tl.multiple_of(v_ptr, [16,16])
    tl.multiple_of(ds_ptr, [16,16])
    tl.multiple_of(dk_ptr, 16)
    tl.multiple_of(du_ptr, 16)
    tl.multiple_of(dv_ptr, 16)
    
    k_block_ptr_2 = k_ptr + expert_id * k_expert_stride + expert_dim_idx * k_expert_dim_stride
    u_block_ptr_2 = u_ptr + expert_id * u_expert_stride + expert_dim_idx * u_expert_dim_stride
    vT_block_ptr_2 = v_ptr + expert_id * v_expert_stride + expert_dim_idx * v_expert_dim_stride
    tl.multiple_of(k_block_ptr_2, [16,16])
    tl.multiple_of(u_block_ptr_2, [16,16])
    tl.multiple_of(vT_block_ptr_2, [16,16])
    k_block_2 = tl.load(k_block_ptr_2, cache_modifier='.ca')
    u_block_2 = tl.load(u_block_ptr_2, cache_modifier='.ca')
    vT_block_2 = tl.trans(tl.load(vT_block_ptr_2, cache_modifier='.ca'))
    ...
    for _ in tl.range(0, BATCH_SIZE, 1, num_stages=None, loop_unroll_factor=None): # loop_unroll_factor=BATCH_SIZE makes it even slower?
        ...
        for seq_start_ in tl.range(0, seq_len, BLOCK_SEQ, num_stages=None):
            ...
            r_block_2 = tl.load(r_block_ptr_2, mask=mask_rows, cache_modifier=".cg", eviction_policy='').to(tl.bfloat16)
            ...
            # dsiluM = (siluM + sigM*(1-siluM)) # silu'(x) = sig + x*sig*(1-sig) = silu+sig(1-silu)
            expertN = r_block_2 * N
            AT = tl.trans(siluM * expertN)
            # backpropagate dA = dS @ V^T, dN = dA * silu(M), dM = dA * N * silu'(M). silu'(x) = sigx+x*sigx*(1-sigx)
            dA = tl.dot(ds_block_2, vT_block_2).to(tl.bfloat16)
            dM = (dA * expertN) * (siluM + sigM*(1.0-siluM))
            dN = (dA * siluM) * r_block_2
        
            dv_acc = tl.dot(AT, ds_block_2, dv_acc)                   # (BLOCK_INTER, HEAD_DIM)
            dk_acc = tl.dot(tl.trans(dM), q_block_2, dk_acc) # (BLOCK_INTER, HEAD_DIM)
            du_acc = tl.dot(tl.trans(dN), q_block_2, du_acc)   # (BLOCK_INTER, HEAD_DIM)
    
    # write accumulated dK, du, dv for this (inter_block, head_dim). all load normally, no transpose load
    dk_block_ptr = dk_ptr + expert_id * k_expert_stride + expert_dim_idx * k_expert_dim_stride + col_ids * k_head_dim_stride
    du_block_ptr = du_ptr + expert_id * u_expert_stride + expert_dim_idx * u_expert_dim_stride + col_ids * u_head_dim_stride
    dv_block_ptr = dv_ptr + expert_id * v_expert_stride + expert_dim_idx * v_expert_dim_stride + col_ids * v_head_dim_stride
...
class FlashMLPFunction(torch.autograd.Function):
    ...
    def forward(ctx, Q, K, U, V, R):
        ...
        Args:
            Q: (batch_size, num_heads, seq_len, head_dim)
            K: (num_kuv_heads, num_expert, expert_dim, head_dim)
            U: (num_kuv_heads, num_expert, expert_dim, head_dim) or None
            V: (num_kuv_heads, num_expert, expert_dim, head_dim)
            R: (batch_size, num_heads, seq_len, num_expert)
        
        Returns:
            output: (batch_size, num_heads, seq_len, head_dim)
        """
        batch_size, num_heads, seq_len, head_dim = Q.shape
        num_kuv_heads, num_expert, expert_dim, _ = K.shape
        num_kuv_groups = num_heads // num_kuv_heads
        if expert_dim % 64 != 0:
            raise ValueError(f"Expert dimension {expert_dim} must be divisible by 64, otherwise some part of kernel will not work")
        ...
        # Launch kernel
        flash_mlp_moe_forward_kernel[grid](
            Q, K, U, V, R, output,
            batch_size, num_heads, seq_len, head_dim, num_expert, expert_dim,
            *Q.stride(),
            *K.stride(),
    ...
    def backward(ctx, grad_output):
        ...
        def grid2(meta):
            ...
            NUM_EXPERT = meta['NUM_EXPERT']
            tiles_per_exp = expert_dim // BLOCK_INTER
            grid_inter = NUM_EXPERT * tiles_per_exp
        ...
        kernel = flash_mlp_moe_backward_kernel_dqdr[grid]
        kernel2 = flash_mlp_moe_backward_kernel_dkdudv[grid2]
        # Launch backward kernel
        kernel(
            Q, K, U, V, R, grad_output,
            dQ, dR,
            batch_size, num_heads, seq_len, head_dim, num_expert, expert_dim,
            *Q.stride(),
            *K.stride(),
        ...
        return dQ, dK, dU, dV, dR, None, None, None


class FlashMLPMoE(torch.nn.Module):
    """
